	...
	fileprivate class func setUserID() {
		...
		if isAuth == true, let currentUser = UserManager.shared.currentUser {
			...
			runAsynchronouslyOnEventProcessingQueue {
				FBSDKAppEvents.setUserID(user_id)
				Amplitude.shared.setUserId(user_id)
				Crashlytics.sharedInstance().setUserIdentifier(user_id)
                Adjust.addSessionCallbackParameter("user_id", value: user_id)
			}

			var userInfo: [String: Any] = [
				"Monkey_gender": currentUser.gender ?? "male",
				"Monkey_age": currentUser.age,
				"Monkey_ban": currentUser.is_banned,
				"current_banana": currentUser.bananas,
				]
