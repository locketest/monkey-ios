//
//  OnepPairMatchViewController+Action.swift
//  Monkey
//
...
extension OnepPairMatchViewController: CountingLabelDelegate {
	func minuteAdded() {
		...
		for _ in 1...18 {
			...
			self.view.insertSubview(emojiLabel, belowSubview: self.addMinuteButton)
		}
	}
	...
	func addFriendSuccess() {
		...
		for _ in 1...130 {
			...
			emojiLabel.font = UIFont.systemFont(ofSize: 39.0)
			let positionX: CGFloat = self.addMinuteButton.frame.origin.x + 30
			emojiLabel.frame = CGRect(x: positionX, y: self.addMinuteButton.frame.origin.y, width: 50, height: 50)
			self.view.insertSubview(emojiLabel, belowSubview: self.addMinuteButton)
