...
class RealmPhoneAuth: JSONAPIObject, JSONAPIObjectProtocol {
	...
	static let requst_subfix = RealmPhoneAuth.type
	...
	func update(attributesJSON: [String:Any], completion: @escaping (_ error: APIError?,_ response : JSONAPIDocument?) -> Void) {
		...
		JSONAPIRequest(url: "\(Environment.baseURL)/api/\(RealmPhoneAuth.api_version)/\(RealmPhoneAuth.requst_subfix)/\(auth_id)", method: .patch, parameters: [
			"data": [
				"id": auth_id,
				"type": "phone_auths",
