...
+ (instancetype)shared {
    static OTDefaultAudioDevice* _shared;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        _shared = [[OTDefaultAudioDevice alloc] init];
    });
    return _shared;
}
...
- (void) setupAudioSession
{
    ...
    NSError *error = nil;
    NSUInteger audioOptions = AVAudioSessionCategoryOptionAllowBluetooth;
#if !(TARGET_OS_TV)
                           ...
                           AudioBufferList *buffer_list)
{
    ...
    AudioBuffer audioBuffer = buffer_list->mBuffers[0];
    
    if (dev->_isMuted) {
        for(int i = 0; i < audioBuffer.mDataByteSize; ++i) {
            ((char *)audioBuffer.mData)[i] = 0x00;
        }
    }

    uint32_t count =
    [dev->_audioBus readRenderData:audioBuffer.mData
                   numberOfSamples:num_frames];
